...
def main():
    path = r'c:\Users\pelto\OneDrive\Desktop\Research Code\mc_raw_zach'
    ...
    for file in filelist:
        ...
        prev_data = deque(maxlen=SEQ_LEN)
            ...
            # prev_data = np.nan_to_num(prev_data, nan=np.nanmean(prev_data), posinf=np.nanmean(prev_data), neginf=np.nanmean(prev_data))
        ...
        for i in df.values:
            ...
            #prev_data = prev_data.astype(float)
        ...
        print(name_no_ext)
        df.to_csv(name_no_ext + "_data.csv", index=False)
